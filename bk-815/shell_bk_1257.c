...
/*void history_add(history **head_ref, char *newline, int *current_index) {

  history *new_node = (history *)malloc(sizeof(history));
  strncpy(new_node->line, newline, strlen(newline));
  new_node->next = NULL;
  if (*head_ref == NULL) {
    *current_index = 0;
    new_node->index = 0;
...
char *history_get(history *head, int find_index) {
