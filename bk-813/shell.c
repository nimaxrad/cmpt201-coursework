...
// CMAKE file?
// should i use write instead of printf?y
...
int history_get_ten_bhistad(history *head,
                            int current_index) { // not used- instead go to
                                                 // history_get_ten
...
void write_int(int n) {
  char buf[12]; // enough for10 digits + '\0'
  int i = 0;
  if (n == 0) {
...
static char *handle_home(const char *path, char *out, size_t cap) {
  if (!path || path[0] != '~') {
    // just copy
    char *path_holder;
    if (path) {
  ...
  const char *rest = path + 1; // only after~
  const char *home = NULL;
  if (*rest == '/' || *rest == '\0') {
    struct passwd *pw = getpwuid(getuid()); // get home
    if (pw && pw->pw_dir)
      home = pw->pw_dir;
  } else {
    // ~u
...
static int builtin_cd(char **argv, int argc, char prevdir[PATH_MAX]) {
  ...
  if (argc == 1) {
    ...
    target = handle_home(argv[1], expanded, sizeof(expanded));
  } else {
    target = argv[1];
  }
...
int main() { //---------------------------------------------------------------------------------------------------------------------------
  struct sigaction sa;
  memset(&sa, 0, sizeof(sa)); // addmemory
  sa.sa_handler = sigint_handler;
  sigemptyset(&sa.sa_mask);
  sa.sa_flags =
      SA_RESTART; // auto-restart some syscalls when possible - stack overflow
    ...
    */
    ...
    if (n == -1 || strlen(line) == 0) {
      ...
      if (pid == 0) {
        ...
        sa_child.sa_handler = sigint_handler; // SIG_DFL;
